...
// Real anchors, not buttons: the homepage HTML has to carry a crawlable link
// with the post title as anchor text. Client-side navigation still happens
// through next/link.
export default function PostCard({ post, priority = false }: PostCardProps) {
  const href = `/leap/${post.slug}`;
  ...
  return (
    <div className="h-auto">
      <article className="bg-white rounded-xl shadow-md overflow-hidden hover:shadow-xl hover:-translate-y-1 transition-all duration-300 group h-auto flex flex-col">
        {heroImage &&
          (post.hasContent ? (
            <Link href={href} className="block w-full overflow-hidden" style={{ background: '#EBF0E6' }}>
              {heroImage}
            </Link>
          ) : (
            <div className="block w-full overflow-hidden" style={{ background: '#EBF0E6' }}>
              {heroImage}
            </div>
          ))}
        <div className="p-6 flex flex-col flex-1">
          ...
          <h3 className="mt-2 text-xl font-semibold text-gray-900 group-hover:text-[#8B6914] transition-colors">
            {post.hasContent ? <Link href={href}>{post.title}</Link> : post.title}
          </h3>
